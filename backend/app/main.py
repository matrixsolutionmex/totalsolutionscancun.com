...
def create_database_tables():
    ...
    "service_opportunities",
        ...
        add_column_if_missing(db, "email_outbox", "template_type", "VARCHAR")
        ...
        db.execute(text("UPDATE users SET status = 'ACTIVE' WHERE (status IS NULL OR status = '') AND email_verified = TRUE"))
        db.execute(text("UPDATE users SET status = 'PENDING_EMAIL' WHERE (status IS NULL OR status = '') AND COALESCE(email_verified, FALSE) = FALSE"))
...
@app.get("/unete-a-la-red", include_in_schema=False)
def professional_network_page():
    return public_information_page("unete-a-la-red.html")
